...
func main() {
	// call solution with testInput
	solution(testInput)
	// call solution with realInput
	solution(realInput)
...
// solution function takes a string and returns nothing
func solution(s string) {
	// split s by new line
	splitByNewLine := splitStringByNewLine(s)
	// total priority
	var totalPriority int
	// iterate over splitByNewLine
	for _, line := range splitByNewLine {
		// split line in half
		half := len(line) / 2
		// find character that exists in both first and second half
		for _, char := range line[:half] {
			if strings.Contains(line[half:], string(char)) {
				// print character
				println(string(char))
				// if lower case, priority is char minus "a" plus 1
				if char >= 'a' && char <= 'z' {
					totalPriority += int(char) - int('a') + 1
				}
				// if upper case, priority is char minus "A" plus 27
				if char >= 'A' && char <= 'Z' {
					totalPriority += int(char) - int('A') + 27
				}
				break
			}
		}
...
func solution2(s string) {
	// split s by new line
	splitByNewLine := splitStringByNewLine(s)
	// group strings into groups of three strings
	var groups [][]string
	// iterate over splitByNewLine
	for i := 0; i < len(splitByNewLine); i += 3 {
		// append group to groups
		groups = append(groups, splitByNewLine[i:i+3])
	}
	// total priority
	var totalPriority int
	// iterate over groups
